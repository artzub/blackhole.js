...
(function() {
    ...
    function getFun(obj, key) {
        return obj && isFun(obj[key]) ? obj[key] : emptyFun;
    }
    ...
    function DataParser() {
        ...
        (function() {
            ...
            parser.getName = function(d) {
                return d.name;
            };

            parser.getCategory = function(d) {
                return d.category;
            };

            parser.getKey = function(d, type) {
                return type == typeNode.child ? env.getChildKey(d) : env.getParentKey(d);
            };

            parser.getChildKey = function(d) {
                return d.key;
            };

            parser.getParentKey = function(d) {
                return d.key;
            };

            parser.getParent = function(d) {
                return d.parent;
            };

            parser.getParentImage = function(d) {
                return d.img;
            };

            parser.getGroupBy = function(d) {
                return d.date;
            };

            parser.getValue = function(d) {
                return 1;
            };

            parser.getParentRadius = function(d) {
                return 25;
            };

            parser.getChildRadius = function(d) {
                return 2;
            }
         ...
         * Create a Category
         * @param c
         * @returns {{key: string, all: number, currents: *, color: d3.rgb, now: Array}}
         * @constructor
         */
        function Category(c) {
            ...
            if (!cat) {
                cat = {
                    ...
                    all: 0,
                    currents: {},
                    color: d3.rgb(parser.childColor(c)),
                    now: []
                };
        ...
        function Node(d, type) {
            var isChild = type == typeNode.child
                , c = isChild
                    ? parser.getCategory(d)
                    : parser.parentColor(parser.getParentKey(d))
                , cat
                , x
                , y
                , w = parser.size[0]
            ...
            return {
                x : x,
                y : y,
                id : type + parser.getKey(d, type),
                size : !isChild ? parser.getParentRadius(d) : parser.getChildRadius(d),
                fixed : true,
                visible : false,
                links : 0,
                type : type,
                ...
                category : cat,
                parent : !isChild ? parser.getParentKey(d) : null,
                img : !isChild ? parser.getParentImage(d) : null,
                nodeValue : d
            };
        }
        ...
        function getParent(d) {
            ...
            if (!d || !(parent = parser.getParent(d)))
                return null;

            key = parser.getParentKey(parent);
            if (!key)
                return null;
        ...
        parser.nodes = function(data) {
            ...
            if (data) {
                ...
                while(--i > -1) {
                    ...
                    n.category.values['_' + n.id] = parser.getValue(d);
                    !n.inserted && (n.inserted = ns.push(n));
    ...
    function Render() {
        ...
        function getLastEvent() {
            return getFun(that, "onGetLastEvent")();
        }

        function getChildNodes() {
            return getFun(that, "onGetChildNodes")();
        }

        function getParentNodes() {
            return getFun(that, "onGetParentNodes")();
        }

        function getParentLabel(d) {
            return getFun(that, "onGetParentLabel")(d);
        }

        function getChildLabel(d) {
            return getFun(that, "onGetChildLabel")(d);
        }

        function getSelectedColor(d) {
            return getFun(that, "onGetSelectedColor")(d);
        }

        function getNodeRadius(d) {
            return getFun(that, "onGetNodeRadius")(d);
        }
        ...
        function draw() {
            ...
            if (!bufCtx) {
                ...
                bufCanvas.width = that.size[0];
                bufCanvas.height = that.size[1];

                bufCtx = bufCanvas.getContext("2d");
                bufCtx.globalCompositeOperation = 'lighter';

                bufCtx.font = "normal normal " + that.fontSize + "px Tahoma";
            ...
            bufCtx.save();
            bufCtx.clearRect(0, 0, that.size[0], that.size[1]);

            if (that.compositeOperation && bufCtx.globalCompositeOperation != that.compositeOperation)
                bufCtx.globalCompositeOperation = that.compositeOperation;
            ...
            if (that.drawChild) {
                ...
                currentCache = that.drawAsPlasma ? neonBallCache : particleImageCache;

                l = n.length;

                if (!that.drawHalo && that.drawTrack) {
                    c = null;
                    i = 100;
                    ...
                    while(--l > -1) {
                        ...
                        drawTail(c, d, Math.floor(d.x), Math.floor(d.y), that.drawVanishingTail);
                    }
                }
                ...
                while(--l > -1) {
                    ...
                    x = Math.floor(d.x);
                    y = Math.floor(d.y);
                    s = getNodeRadius(d) * (that.drawHalo ? that.drawAsPlasma ? 8 : 10 : .8);

                    if (that.drawHalo && that.drawTrack)
                        drawTail(c, d, x, y, that.drawVanishingTail);
                ...
                if (!that.drawHalo && beg) {
                    bufCtx.stroke();
                    bufCtx.fill();
                }
            }

            if (that.drawParent || that.drawLabel) {
                ...
                while(--l > -1) {
                    ...
                    if (that.drawParent) {
                        ...
                        if (that.drawPaddingCircle) {
                            bufCtx.beginPath();
                            bufCtx.strokeStyle = "none";
                            bufCtx.fillStyle = "#ff0000";
                            bufCtx.arc(x, y, s + that.padding, 0, PI_CIRCLE, true);
                            bufCtx.closePath();
                            bufCtx.fill();
                            bufCtx.stroke();
                        }
                        ...
                        drawPI = that.drawParentImg && d.img && d.img.width;

                        bufCtx.beginPath();
                        bufCtx.strokeStyle = "transparent";
                        bufCtx.fillStyle = draPI ? "transparent" : c;
                        bufCtx.arc(x, y, s, 0, PI_CIRCLE, true);
                        bufCtx.closePath();
                        bufCtx.fill();
                        bufCtx.stroke();
                    ...
                    if (that.drawParentLabel) {
                        c = d.flash ? "white" : "gray";

                        bufCtx.fillStyle = c;
                        bufCtx.fillText(getParentLabel(d), x, y + s * 1.5);
                    }
                }
            }
    ...
    d3.blackhole = function(node) {
        ...
        /*var setting = {
            ...
            , asPlasma : false // particle draws based on gradient method by canvas
            , showTrack : true // show particles tracks
            , vanishingTail : true // particles track is vanishing
        };*/
        ...
        (function() {
            ...
            bh.childColor = function(arg) {
                if(!arguments.length || !isFun(arg))
                    return parser.childColor;
                parser.childColor = arg;
                return bh;
            };

            bh.parentColor = function(arg) {
                if(!arguments.length || !isFun(arg))
                    return parser.parentColor;
                parser.parentColor = arg;
                return bh;
            };

            regStartWith = new RegExp(/^get/);

            d3.map(parser).keys().forEach(function(k) {
                if(!regStartWith.test(k))
                    return;

                var fn = k.replace(regStartWith, "");
                fn = fn.replace(new RegExp("^" + fn[0]), fn[0].toLowerCase);
...
//            , onRecalc : null
        ...
        (function() {
            ...
            bh.onFinished = func("onFinished");
            bh.onStarting = func("onStarting");
            bh.onStarted = func("onStarted");
        ...
        bh.runShow = function(adata, w, h) {
            ...
            data = parser.nodes(adata);

            processor.boundRange = d3.extent(adata.map(parser.getGroupBy));

            var layer = d3.select(parentNode);
            ...
            render.setFontSize(setting.sizeUser / 2);
